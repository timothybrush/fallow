...
const OUTPUT_ROOT = process.env.FALLOW_GENERATION_OUTPUT_ROOT
  ? resolve(process.env.FALLOW_GENERATION_OUTPUT_ROOT)
  : null;
const SCHEMA_PATH = process.env.FALLOW_CODEGEN_OUTPUT_SCHEMA
  ? resolve(process.env.FALLOW_CODEGEN_OUTPUT_SCHEMA)
  : resolve(REPO_ROOT, "docs", "output-schema.json");
...
const outputPath = (path) =>
  OUTPUT_ROOT ? resolve(OUTPUT_ROOT, relative(REPO_ROOT, path)) : path;
...
async function writeAll(artifacts) {
  for (const path of OUTPUT_CONTRACT_PATHS) {
    ...
    await writeFile(target, artifacts.outputContract, "utf8");
  }
  const issueTypesTarget = outputPath(ISSUE_TYPES_PATH);
  await mkdir(dirname(issueTypesTarget), { recursive: true });
  await writeFile(issueTypesTarget, artifacts.issueTypes, "utf8");
  const initializationOptionsTarget = outputPath(LSP_INITIALIZATION_OPTIONS_PATH);
  await mkdir(dirname(initializationOptionsTarget), { recursive: true });
  await writeFile(
    initializationOptionsTarget,
    artifacts.lspInitializationOptions,
    "utf8",
  );
  const packageJsonTarget = outputPath(PACKAGE_JSON_PATH);
  await mkdir(dirname(packageJsonTarget), { recursive: true });
  await writeFile(packageJsonTarget, artifacts.packageJson, "utf8");
}
...
if (mode === "--check") {
  ...
  for (const path of [
    ...
    process.stdout.write(`Wrote ${outputPath(path)}\n`);
  }
}
